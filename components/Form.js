...
const Form = ({handleChange, form, handleSubmit}) => {
    return (  
        <form onSubmit={(e)=>e.preventDefault} autoComplete="off" className={styles.form}>
            ...
            <input className={`${styles.button} ${ (!form.nickname|| form.password.length<8 || form.password.length>50 || form.nickname.length>20) ? styles.disabled : ""} ${styles.form_el} `}  type="button" value="Sign Up" onClick={handleSubmit}/>
            <input className={`${styles.button} ${ (!form.nickname|| form.password.length<8 || form.password.length>50 || form.nickname.length>20) ? styles.disabled : ""} ${styles.form_el}`} type="button" value="Sign In" onClick={handleSubmit}/>
        </form>
    );
}
